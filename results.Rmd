...
```
```{r}
fig <- plot_ly(data=vaccine_tb_rate, x = ~tb_infection_rate, y=~continent,frame = ~year, type='box',color=~continent, colors = 'Spectral')
fig %>% layout(
  plot_bgcolor='#BBBBBB',
  title = "Infection Rate by Continent",
  xaxis = list(title='Continent')
...
fig1 <- plot_ly(data=vaccine_tb_rate, 
        ...
        layout(title = "Vaccination Rate by Continent",
               yaxis = list(title='Continent'),
               xaxis = list(range=c(0,100)),
               showlegend = FALSE)

vaccine_graph <- vaccine_tb_rate %>% 
        select(continent, year, tb_infection_rate) %>% 
        replace(is.na(.), 0) %>% 
        group_by(continent, year) %>% 
        summarize(tb_infection_rate = mean(tb_infection_rate))
...
fig2 <- vaccine_tb_rate %>% 
        ...
        plot_ly(x = ~year, 
                ...
                mode = "lines+markers",
                colors = "Spectral") %>% 
        layout(title = "Incidence Rate among Years by Continents",
...
fig <- subplot(fig1, fig2, nrows = 1, margin = 0.03) %>% 
      layout(plot_bgcolor='#BBBBBB', 
        title = "Vaccination Rate and Incidence Rate Among Continents",
       xaxis = list( 
         zerolinecolor = '#ffff', 
         zerolinewidth = 2, 
         gridcolor = 'ffff'), 
...
fig <- plot_ly(data=vaccine_tb_group, colors='Spectral')
